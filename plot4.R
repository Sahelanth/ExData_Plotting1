...
#Make a plot and save it as plot4.png
png(file = "plot4.png")
with(df2, par(mfrow = c(2, 2)))

#Top left plot
plot(df2$datetime, df2$Global_active_power, xlab = "", ylab= "Global Active Power (kilowatts)", type="l")

#Top right plot
plot(df2$datetime, df2$Voltage, xlab = "datetime", ylab= "Voltage", type="l")

#Bottom left plot
  plot(df2$datetime, df2$Sub_metering_1, xlab = "", ylab= "Energy sub metering", type="l")
      )
  #Add the other two sub metering sets and a legend to bottom left plot
...
#Bottom right plot
plot(df2$datetime, df2$Global_reactive_power, xlab = "datetime", ylab= "Global_reactive_power", type="l")
